...
공공데이터포털(data.go.kr)이 처음인 분을 위한 그림 가이드다. 이 MCP 서버(`@opendata-kr/narajangteo-corpinfo-mcp`)를 쓰려면 공공데이터포털에서 **나라장터 사용자정보서비스**를 활용신청하고, 발급받은 인증키를 서버 설정에 넣어야 한다.

> [!IMPORTANT]
> **서비스키를 어딘가에서 발급받았다고 바로 되는 게 아니다.**
> 공공데이터포털 인증키는 계정당 하나이고 여러 API가 그 키 하나를 함께 쓴다. 그렇지만 **각 API는 저마다 따로 활용신청을 해서 `[승인]`을 받아야** 그 API 주소에서 인증된다. 즉 키를 가지고 있어도 **나라장터 사용자정보서비스**를 활용신청하지 않았다면 이 서버의 호출은 인증 오류(결과코드 30)로 막힌다. 형제 서비스(입찰공고·낙찰·사전규격 등)를 신청했더라도 사용자정보서비스는 별도로 신청해야 한다.
...
[공공데이터포털](https://www.data.go.kr)에서 **나라장터 사용자정보서비스**를 검색해 **조달청\_나라장터 사용자정보 서비스**를 찾는다.

![검색 결과에서 오픈 API 탭을 누르고, 조달청_나라장터 사용자정보 서비스의 활용신청 버튼을 찾는 화면](images/01-search.png)

1. **오픈 API** 탭을 누른다.
2. 제공기관이 **조달청**인 **조달청\_나라장터 사용자정보 서비스**를 찾는다(①). 이름이 비슷한 서비스(입찰공고·낙찰·계약 등)가 많으니 **사용자정보 서비스**가 맞는지 확인한다.
3. 그 줄의 **활용신청** 버튼을 누른다(②). (제목을 눌러 상세 페이지로 들어가도 된다.)

바로 상세 페이지를 열려면 아래 링크를 써도 된다: <https://www.data.go.kr/data/15129466/openapi.do>
...
1. 서비스 이름이 **조달청\_나라장터 사용자정보 서비스**가 맞는지 확인한다(①).
2. **활용신청** 버튼을 누른다(②).
...
1. **활용목적** (필수): 라디오 버튼에서 하나를 고르고, **바로 아래 입력란(250자)에도 목적을 직접 적는다.** 라디오와 입력란이 둘 다 필수라, 입력란을 비우면 `활용목적을 입력해주세요`가 빨간 글씨로 뜨고 신청이 넘어가지 않는다.
   ...
   - **입력란**: 목적을 한 줄로 적는다. 예: `narajangteo-corpinfo-mcp(https://github.com/opendata-kr/narajangteo-corpinfo-mcp)에서 조달업체 자격·프로파일 조회에 사용`
2. **첨부파일** (없음): 이 서비스는 위치정보를 다루지 않으므로 첨부할 파일이 없다. 그냥 넘어간다. 첨부파일은 위치정보를 포함한 서비스를 신청하는 사업자가 '위치기반서비스사업신고필증'을 낼 때만 쓴다.
3. **상세기능정보** (그대로 둠): 이 API가 제공하는 오퍼레이션 목록이 **기본으로 모두 체크**되어 있다. 손대지 말고 전부 선택된 채로 둔다. 사용자정보서비스는 조달업체 기본정보·등록업종·공급물품·부정당제재와 수요기관정보 오퍼레이션을 제공한다.
...
![마이페이지 활용신청 현황에서 조달청_나라장터 사용자정보 서비스가 승인 상태로 목록에 있는 화면](images/03-approved-list.png)

1. 상태가 `[승인]`인지 확인한다(①).
2. **조달청\_나라장터 사용자정보 서비스**가 목록에 있는지 확인한다(②).
...
- **인증 오류(결과코드 30)**: 두 가지를 확인한다. ① **Decoding** 키가 아니라 **Encoding** 키를 넣지 않았는지, ② 3단계 목록에서 **나라장터 사용자정보서비스**가 `[승인]` 상태로 있는지. 다른 API용으로 발급한 키만 있고 이 API를 활용신청하지 않았다면 이 오류가 난다.
